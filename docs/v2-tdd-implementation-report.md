...
- 建設現場向け PoC の横分割ページを 1 行 30 列へ復元する V2 専用マージ
- `2ページ同一列` / `6ページ同一列` を安全に縦連結する V2 専用マージ
- raw 列を保持したまま、`正規化入場*` / `正規化退場*` / `*_分` / `時刻正規化状態` / `時刻確認メモ` を追加
...
これにより、V2 側で「どの raw 列を時刻正規化対象にするか」を明示できるようにしました。
...
Power Query では raw 転記を優先し、正規化列は Excel 読込後に PowerShell 側で追加しています。
...
- [construction-site-transfer-proposal.md](/C:/Work_Codex/PDF2Excel/docs/construction-site-transfer-proposal.md)
...
今回で「raw を壊さず、時刻を分析しやすくする」基盤までは入りました。  
まだ残るのは次です。
...
ただし、今回の目的だった `VER2` の raw 転記安定化、多ページマージ、時刻正規化、V1 非破壊は満たしています。
