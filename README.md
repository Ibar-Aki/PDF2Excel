...
- `outputColumnNames` を設定した V2 プロファイルでは、Result の列名を `項目1..N` ではなく帳票ヘッダー由来の名称で出力できます。
- V2 は高さ方向のヘッダーずれや軽微な文言揺れを吸収し、取り込みを継続しつつ `Review` に警告を残せます。
...
- `4`: 出力フォルダを開く
- `5`: プロファイルフォルダを開く
- `6`: プロファイル雛形を作成 (`VER2` では設定ウィザードで主要設定を順番に入力し、必要に応じてサンプル PDF 1 件から下書きを補助生成)
- `7`: ログフォルダを開く
- `8`: 終了
- `9`: プロファイルを選ぶ
- `0`: 環境チェック
