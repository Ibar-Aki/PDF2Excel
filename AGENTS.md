...
- `run_pdf2excel.bat` を変更した場合は、`handoff/PDF2Excel_Minimal/run_pdf2excel.bat` と `scripts/build_handoff_package.ps1` への影響を確認してください。
- `scripts/run_pdf2excel_menu.ps1` を変更した場合は、BAT 直実行の回帰確認を行ってください。
- 新しい帳票プロファイルを追加した場合は、対応するサンプル PDF と少なくとも 1 件のテストを追加してください。
- `samples/pdf` 配下の既存サンプルを意図せず削除しないでください。生成スクリプトは管理対象ディレクトリだけを更新してください。
