...
- 作成日: 2026-03-20 00:45 JST
- 作成者: Codex (GPT-5)
- 更新日: 2026-03-20

## サマリー

- 実施日時: 2026-03-20 00:45:23 JST - 2026-03-20 00:45:25 JST
...
| 6 | Protect-MessagePaths は登録済みパスをマスクできる | 成功 | 28 ms | 出力先=...\logs\run_001.log / staging=...\staging\a.pdf |
| 7 | Get-ProfileOutputColumnNames は接頭辞と元ファイル列を並べる | 成功 | 10 ms | SourceFile,Column1,Column2,Column3 |
| 8 | Get-ProfileConfiguration は既定プロファイルを読み込む | 成功 | 87 ms | 標準30列プロファイル / 30 |
...
| 13 | Get-RunErrorGuidance は次アクションを返す | 成功 | 14 ms | 別の実行が完了するまで待ってから再実行してください。必要ならログフォルダで直近の実行状況を確認してください。 |
...
| 16 | Get-ProfileConfiguration は日本語在庫一覧プロファイルを読み込む | 成功 | 43 ms | 日本語在庫一覧プロファイル / 10 |
...
| 26 | テストとビルド導線に Bypass が残っていない | 成功 | 27 ms | tests / build 呼び出しの Bypass 除去を確認 |
...
| 36 | テンプレート整合性マニフェストは必要ファイルを持つ | 成功 | 8 ms | template-integrity.json の主要エントリを確認 |
...
| 38 | Write-Log は INFO で詳細パスを伏せ DEBUG を抑止する | 成功 | 22 ms | INFO では path mask / DEBUG 抑止を確認 |
...
| 42 | Normalize-TimeText は全角コロンを半角時刻へ正規化する | 成功 | 27 ms | 08:00 / 480 |
| 43 | Normalize-TimeText は時分表記と空白込みを正規化する | 成功 | 1 ms | 09:15 / 555 |
| 44 | Normalize-TimeText は時のみ表記を 00 分補完する | 成功 | 1 ms | 18:00 / 1080 |
| 45 | Normalize-TimeText は Excel 時刻比率の文字列も正規化する | 成功 | 21 ms | 09:15 / 555 |
...
| 49 | Normalize-TimeText は 24:00:00 を 24:00 として扱う | 成功 | 6 ms | 24:00 / 1440 |
| 50 | Get-ResultOutputColumnNames は V2 で正規化列を追加する | 成功 | 14 ms | 正規化入場2_分,正規化退場2,正規化退場2_分,時刻正規化状態,時刻確認メモ |
| 51 | Get-NormalizedTimeColumnDefinitions は Review raw 列名を返す | 成功 | 9 ms | 正規化入場1_raw,正規化退場1_raw |
| 52 | Get-ReviewReasonCategories は HEADER_MISMATCH を先頭に重複なく返す | 成功 | 53 ms | HEADER_MISMATCH,TIME_MULTI,TIME_MISSING,TIME_INVALID |
| 53 | Get-StagingQueryFormula は V2 で canonical sameHeader と曖昧分離を考慮する | 成功 | 37 ms | canonical sameHeader / ambiguity split / review raw / reason categories |
| 54 | Get-ReviewQueryFormulaV2 は ReasonCategory を展開対象に含める | 成功 | 12 ms | ReasonCategory expansion ready |
| 55 | run_pdf2excel.bat は ASCII のみで構成される | 成功 | 8 ms | ASCII のみを確認 |
