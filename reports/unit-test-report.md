...
- 作成日: 2026-03-20 09:31 JST
- 作成者: Codex (GPT-5)
- 更新日: 2026-03-20

## サマリー

- 実施日時: 2026-03-20 09:31:24 JST - 2026-03-20 09:31:26 JST
...
| 9 | Get-StagingQueryFormula は入力フォルダと判定ロジックを埋め込む | 成功 | 30 ms | Folder.Files と COLUMN_OVERFLOW を確認 |
...
| 18 | Get-ProfileConfiguration は生データ転記サンプルプロファイルを読み込む | 成功 | 10 ms | 生データ転記サンプルプロファイル / 30 |
...
| 25 | 正式運用 BAT は V2 Secure を起動し RemoteSigned に統一されている | 成功 | 4 ms | run_pdf2excel.bat は V2 Secure、開発導線も RemoteSigned を確認 |
| 26 | テストとビルド導線に Bypass が残っていない | 成功 | 36 ms | tests / build 呼び出しの Bypass 除去を確認 |
...
| 37 | 日本語コンソール出力スクリプトは UTF-8 初期化と保存形式を持つ | 成功 | 10 ms | run/build/scaffold scripts の UTF-8 初期化と build_handoff_package.ps1 の BOM を確認 |
| 38 | Write-Log は INFO で詳細パスを伏せ DEBUG を抑止する | 成功 | 22 ms | INFO では path mask / DEBUG 抑止を確認 |
| 39 | Write-Log は DEBUG 指定時だけ詳細パスを出力する | 成功 | 16 ms | DEBUG では詳細パスを確認 |
| 40 | ログ要約関数は件数と代表ファイル名を返す | 成功 | 12 ms | 4 件 (alpha.pdf, beta.pdf, gamma.pdf ほか 1 件) |
| 41 | run_pdf2excel.ps1 は待機メッセージを表示する | 成功 | 2 ms | 待機メッセージを確認 |
| 42 | Normalize-TimeText は全角コロンを半角時刻へ正規化する | 成功 | 29 ms | 08:00 / 480 |
| 43 | Normalize-TimeText は時分表記と空白込みを正規化する | 成功 | 1 ms | 09:15 / 555 |
| 44 | Normalize-TimeText は時のみ表記を 00 分補完する | 成功 | 1 ms | 18:00 / 1080 |
| 45 | Normalize-TimeText は Excel 時刻比率の文字列も正規化する | 成功 | 22 ms | 09:15 / 555 |
| 46 | Normalize-TimeText は 24:00 を有効時刻として扱う | 成功 | 1 ms | 24:00 / 1440 |
| 47 | Normalize-TimeText は 24:30 を範囲外扱いにする | 成功 | 4 ms | INVALID / 時刻の範囲外です。 |
...
| 51 | Get-NormalizedTimeColumnDefinitions は Review raw 列名を返す | 成功 | 10 ms | 正規化入場1_raw,正規化退場1_raw |
| 52 | Get-ReviewReasonCategories は HEADER_MISMATCH を先頭に重複なく返す | 成功 | 57 ms | HEADER_MISMATCH,TIME_MULTI,TIME_MISSING,TIME_INVALID |
| 53 | Get-StagingQueryFormula は V2 で canonical sameHeader と曖昧分離を考慮する | 成功 | 49 ms | canonical sameHeader / ambiguity split / review raw / reason categories |
| 54 | Get-ReviewQueryFormulaV2 は ReasonCategory を展開対象に含める | 成功 | 11 ms | ReasonCategory expansion ready |
| 55 | run_pdf2excel.bat は ASCII のみで構成される | 成功 | 9 ms | ASCII のみを確認 |
